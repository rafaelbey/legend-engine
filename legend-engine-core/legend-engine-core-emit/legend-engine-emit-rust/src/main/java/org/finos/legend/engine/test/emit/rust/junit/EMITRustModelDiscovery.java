...
/**
 * Mirrors {@code EMITModelDiscovery} from {@code legend-engine-emit-junit}
 * (package-private there, so we re-implement). Walks the given
 * classpath root for {@code *.emit.yaml} files and returns them as
 * absolute {@link Path}s.
 */
final class EMITRustModelDiscovery
{
    private EMITRustModelDiscovery()
    {
    }
    ...
    static MutableList<Path> findEmitYamls(String classpathRoot)
    {
        ...
        if (classLoader == null)
        {
            classLoader = EMITRustModelDiscovery.class.getClassLoader();
        }
        try
        {
    ...
    private static void walkRoot(URL url, Consumer<? super Path> consumer)
    {
        ...
        {
            throw new UnsupportedOperationException("EMIT Rust JUnit discovery currently only supports file: classpath roots, but got: " + url);
        }
        Path root;
        try
        {
