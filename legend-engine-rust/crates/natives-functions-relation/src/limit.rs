...
///
/// The input TDS is read via [`read_parsed_tds`]; the chosen row prefix
/// plus the source column metadata are re-emitted as a fresh `TDS` via
/// [`alloc_tds_from_parsed`] (rows = source of truth, column types
